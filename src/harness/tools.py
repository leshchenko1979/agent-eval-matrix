...
def grep(ctx: RunContext[FileEditDeps], pattern: str, file_path: str) -> list[str]:
    _bump_tool("grep")
    increment_eval_metric("grep_calls", 1)
    set_eval_attribute("used_grep", True)
    try:
...
def read_file(ctx: RunContext[FileEditDeps], file_path: str) -> str:
    ...
    increment_eval_metric("read_file_calls", 1)
    try:
        target = resolve_workspace_path(ctx.deps.workspace, file_path)
    except SandboxError as exc:
        return str(exc)
    ...
    increment_eval_metric("bytes_read", len(content.encode("utf-8")))
    return content
...
def str_replace(
    ...
    *,
    ...
    increment_eval_metric("str_replace_calls", 1)
    try:
        target = resolve_workspace_path(ctx.deps.workspace, file_path)
    except SandboxError as exc:
        return str(exc)
    ...
    if count == 0:
        increment_eval_metric("str_replace_failures", 1)
        if strict_messages:
            return (
    ...
    set_eval_attribute("str_replace_succeeded", True)
...
def str_replace_fuzzy(
    ...
    increment_eval_metric("str_replace_fuzzy_calls", 1)
    try:
        target = resolve_workspace_path(ctx.deps.workspace, file_path)
    except SandboxError as exc:
        return str(exc)
    ...
    if err:
        increment_eval_metric("str_replace_fuzzy_failures", 1)
        return err
